...
buildListM :: Fold_ f m r -> ListM f m r 
buildListM = \phi -> phi Construct Wrap Done

foldListM  :: (Functor f, Monad m) => ListM f m t -> Fold_ f m t
foldListM lst construct wrap done = loop lst where 
   loop = \case Wrap mlst  -> wrap (liftM loop mlst) 
...
foldProducer :: Monad m => Producer a m r -> Fold_ (Of a) m r
foldProducer prod = \construct wrap done -> 
...
buildProducer :: Monad m =>  Fold_ (Of a) m r -> Producer a m r 
buildProducer phi = phi (\(a :> p) -> yield a >> p) M return
...
iterT2 phi als = als phi join return

folded'' :: (Functor f, Monad m) => (f (m a) -> m a) -> ListM f m a -> m a
folded'' phi ls = iterT2 phi (foldListM ls)
